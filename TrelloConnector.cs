...
namespace trello_pdd
{
  public class TrelloConnector
  {
    ...
    public Card CreateTrelloCardFromSubTask(Task subTask, string listId)
    {
      var response = _httpClient.PostAsync($"{_baseAddress}/cards?name={subTask.Title}&desc={subTask.DescriptionText}&idList={listId}&{_authenticationParameters}",null);
      // since we have a console application, there is no need for async processing
      response.Wait();
      if (response.Result.IsSuccessStatusCode)
      {
      ...
      {
        WriteLine($"Error while writing card '{subTask.Title}': {response.Result.ReasonPhrase}");
      }
