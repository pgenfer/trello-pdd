    ...
    namespace trello_pdd
    {
      class Program
      {
        ...
        static void Main(string[] args)
        {
          ...
          // check if parent card has this subtask already
          ...
          foreach (var subTaskToCreate in subTasksToCreate)
          {
            if (subTaskToCreate.Value.Count > 0)
            {
              ...
              foreach (var subTask in subTaskToCreate.Value)
              {
                ...
                var subCard = trelloConnection.CreateTrelloCardFromTask(subTask, listId);
                trelloConnection.AddCardAsAttachment(subTaskToCreate.Key, subCard);
              }
            }
          }

             // now create trello cards for all tasks that do not have a parent id
            foreach(var rootTask in tasks.Where(x => x.ParentNumber == null))
            {
                var doesAlreadyExist = cards.Any(x => string.Compare(x.Title,rootTask.Title,true) == 0);
                if(!doesAlreadyExist)
                {
                    trelloConnection.CreateTrelloCardFromTask(rootTask, listId);
                }
            }
